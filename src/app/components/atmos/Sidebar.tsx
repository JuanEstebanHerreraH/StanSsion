...
export function Sidebar({ active, onNavigate, lang, onLangToggle, tr }: SidebarProps) {
  ...
  return (
    <aside style={{
      ...
      {/* Download desktop app — always opens the latest GitHub release page */}
      <div style={{ padding: "0 12px 12px" }}>
        <a href="https://github.com/JuanEstebanHerreraH/StanSsion/releases/latest"
          target="_blank" rel="noopener noreferrer"
          title={lang === "es" ? "Descargar la última versión de escritorio (Windows .exe · 64-bit)" : "Download the latest desktop version (Windows .exe · 64-bit)"}
          style={{ width: "100%", display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", borderRadius: 9, boxSizing: "border-box",
            border: `1px solid ${t.border}`, background: t.surface, color: t.textSecondary, cursor: "pointer", fontSize: 12.5, fontWeight: 500, textAlign: "left", textDecoration: "none", ...ui }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = t.surfaceHover; (e.currentTarget as HTMLElement).style.color = t.textPrimary; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = t.surface; (e.currentTarget as HTMLElement).style.color = t.textSecondary; }}>
